...
- **Model calls go through Vercel AI Gateway by default.** `AI_GATEWAY_API_KEY` (or `VERCEL_OIDC_TOKEN`) is the credential; `ANTHROPIC_API_KEY` is not a substitute. OpenRouter is a second generate arm: `OPENROUTER_API_KEY` plus `--model thinkingmachines/inkling:free` (or `openrouter/…`). The built-in loop, raster propose/critique, and the judge use namespaced ids (`anthropic/…`, `google/…`). Codex and Claude Code harness spawns get scratch-local / child-env routing so they do not rewrite `~/.codex` or `~/.claude`.
