...
Generated by `scripts/vocabulary-sheet.ts` from a `iconsmith parts` extraction of 199 parts, 61 of which a name reaches. The names themselves live in `src/parts/vocabulary.ts`, one entry per mark with the canonical drawing it was read off; `nameParts` matches them onto an extraction by shape, so they survive a re-run.
...
| `oblique` ⚠ | 7×5.5 | 25 | 36 | 0/90/180/270 | 13/36 | airdrop, airdrop-2, apple-newton |
