...
Use the calling workflow's repair limit; no separate deadline or correction allowance is implied. Retain the previous candidate before each repair. Use only the supplied reference drawings; missing vocabulary is not permission to copy another set.
...
The table describes the bundled 24px family. The prepared `revision.json` is authoritative for every style value, including the default radius. Read its selected master and policy before drawing; source-checkout defaults can differ.
...
- **`rect`**: bodies, screens, cards, frames. `r<n>` asks for a radius; the nearest legal tier for that shape is what gets drawn. Defaults to the selected spec's `radius` (3 in the bundled Blode revision).
...
For a requested star, inspect the supplied Blode siblings and preserve the intended silhouette. Do not substitute a diamond or chevron for an unsupported construction.
