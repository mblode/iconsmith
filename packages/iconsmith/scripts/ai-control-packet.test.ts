...
const libraryUnavailable = [
  ...
  "icons-data/_concepts.json",
].some((file) => !existsSync(path.join(blode, file)));

test.skipIf(!existsSync(path.join(blode, "icons-svg/folder-1.svg")))(
  "host corruption carries a measured objective probe",
  async () => {
    const svg = readFileSync(
      path.join(blode, "icons-svg/folder-1.svg"),
      "utf-8"
    );
...
test.skipIf(libraryUnavailable)(
  "builds blinded packets from actual canonical library sources",
  async () => {
    const out = path.join(
...
test.skipIf(libraryUnavailable)(
  "builds independent qualification controls across paint and native-size strata",
  async () => {
    const out = path.join(
    ...
    const presentations = allRows.filter(
