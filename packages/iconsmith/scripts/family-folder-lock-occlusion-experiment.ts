/** Bounded existing-DSL occlusion experiment for distinct filled folder-lock masters. */
...
const FOLDER_LOCK_OCCLUSION_CANDIDATES: readonly Candidate[] = [
  {
    id: "16-a-host-clearance",
    master: 16,
    program: `icon folder-lock-occlusion-16
finish filled
part folder-1-filled-0 at 2,3 scale 1
hole rect 13.75,9.25 10.25x12.75 r2
part lock-filled-0 at 14.75,10.25 scale 0.56`,
  },
  {
    id: "24-a-host-clearance",
    master: 24,
    program: `icon folder-lock-occlusion-24
finish filled
part folder-1-filled-0 at 2,3 scale 1
hole rect 13.75,9.75 10.25x11.75 r2
part lock-filled-0 at 14.75,10.75 scale 0.5`,
  },
] as const;
...
const buildFolderLockOcclusionExperiment = async (options: {
  library: string;
  out: string;
}) => {
  ...
  const packet = createFamilyReferencePacket({
    concept: "folder-lock-occlusion-experiment",
    excludedFamilies: [],
  ...
  const base = () =>
    createStyleRevision({
      ...
      id: "wave-d-folder-lock-occlusion-experiment",
      masters: { "16": { ...SPEC, size: 16 }, "24": { ...SPEC, size: 24 } },
      parts: [],
      policy: DEFAULT_POLICY,
      references: [],
      rubric: "Development-only occlusion experiment; no visual qualification.",
    });
  const boardRows = [];
  const outputs = [];
  for (const candidate of FOLDER_LOCK_OCCLUSION_CANDIDATES) {
  ...
  if (!(distinct.icon && distinct.svg)) {
    throw new Error(
      "The 16px and 24px occlusion masters must have distinct programs and SVGs"
    );
  }
  const outputPaintedGroups = outputs.map(({ id, svgFile }) => ({
    id,
    paintedPathCount: (
      readFileSync(path.join(options.out, svgFile), "utf-8").match(
        /<path\b/gu
      ) ?? []
    ).length,
  }));
  if (
    outputPaintedGroups.some(({ paintedPathCount }) => paintedPathCount !== 2)
  ) {
    throw new Error(
      "Host clearance and admitted lock must remain two painted groups"
    );
  }
  const body = {
    ...
    candidateLimit: {
      actualPerMaster: Object.fromEntries(
        [16, 24].map((master) => [
          master,
          FOLDER_LOCK_OCCLUSION_CANDIDATES.filter(
    ...
    observations: {
      localBoardInspection:
        "Both native light/dark rows now expose a recognizable lock silhouette with visible shackle and body counter. The bounded host cutter leaves a long horizontal folder ledge above the lock and a square inward shoulder; the 16px relationship is crowded. These are retained optical limitations, not a visual pass.",
      topology:
        "Hole counts are raw 64/128/192 antialias-threshold observations; they are not a forced two-hole gate.",
      visualLimitations: [
        "The clearance is a rounded rectangle rather than the purpose-redrawn curved host termination used by folder-shield-filled.svg.",
        "The 16px topology changes from two holes to one at the 192 threshold and splits into three ink components there.",
        "The 24px topology reports one, two, then one hole across thresholds 64, 128 and 192.",
      ],
    },
    operationOrder: {
      assertion:
        "Each program places the folder, attaches a bounded hole cutter to that current host group, then adds the admitted lock as a separate later group. There is no union, so the host cutter does not subtract from the lock.",
    ...
    sourceAvailability: {
      exactFolderLockFilled: "absent",
      inspectedExpectedPath: path.join(
        options.library,
        "icons-svg/folder-lock-filled.svg"
      ),
    },
...
if (process.argv[1]?.endsWith("family-folder-lock-occlusion-experiment.ts")) {
  const out = process.argv[2];
  if (!out) {
    throw new Error(
      "Usage: family-folder-lock-occlusion-experiment.ts <new-output-directory> [blode-icons-package]"
    );
  }
  const library =
    process.argv[3] ??
  ...
  await buildFolderLockOcclusionExperiment({ library, out });
}
