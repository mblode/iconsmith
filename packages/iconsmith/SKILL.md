...
One skeleton, two paints. Keep open arc and line scaffolds identical across paints: the compiler expands their ink in filled mode. Replacing a line with a default-radius rectangle changes its end geometry and can pinch a shackle or create a bulge. Outlined is a centre-line stroke. Filled is that stroke expanded into a solid whose outer edge is the ink the outline already occupied. Interiors that were canvas become `hole`s.
...
When the selected spec declares `detailStroke`, append `detail` to a line to use that measured weight, for example `line 8,12 16,12 detail`. The surrounding ring can retain the normal stroke. The role also works with `r`, `solid`, filled line expansion and outlined trim, and remains fixed through transforms. Use it only when the supplied neighbors support lighter detail. Do not invent a numeric width; a style without `detailStroke` refuses this role. For filled counters, build a detail line as a filled cutter and subtract it explicitly. Author both paints; automatic twin adaptation does not infer detail optics.
