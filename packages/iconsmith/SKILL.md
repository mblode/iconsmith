...
npx iconsmith new database --mixture -o database.svg
```

`iconsmith new --mixture` is the product path for a name the set does not draw: cheap host experts (compile / mark / analog) first, the gateway or OpenRouter agent only if they fail. Third-party packs (Lucide, Heroicons, Tabler, Remix) contribute *names* for inventory, never drawings. `iconsmith improve --control analog --treatment agent --class pack-inventory --concepts …` is the A/B that updates that routing table. Do not paste Lucide path data into a program.
