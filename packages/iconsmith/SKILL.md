...
In a repository checkout, run `npm ci` and `npm run build:local` first, then use `npm run --silent iconsmith --` in place of `iconsmith` below. The isolated agent harness already places the built `iconsmith` executable on PATH. There is no published npm package or global installation.
...
5. Look at the SVG rendered at 16px. Lint checks the spec, not the drawing: a centred rectangle of the right size lints perfectly and is an icon of nothing. Record your inspection; independent AI review determines acceptance under the frozen rubric.
6. Finalize the exact inspected artifacts for independent review. Compiler correction and named visual repair have separate bounded allowances under the original deadline. Missing paperwork requests finalization only; uncertain evidence requests clarification only. A clean render alone is not acceptance.

```bash
iconsmith draw cloud-check.icon > cloud-check.svg
iconsmith lint cloud-check.svg --keyline wide
iconsmith --output json lint cloud-check.svg   # machine-readable
