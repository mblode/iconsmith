...
import { studioBudgetSchema, studioRequestSchema } from "./types.ts";
...
describe("the default per-icon ceiling", () => {
   ...
   *
   * Note that `PAIR_TOURNAMENT_RESERVE_USD` currently under-reports the field
   * by exactly one `PROPOSAL_RESERVE_USD` — the arm table hands `image-agent`
   * `IMAGE_PAIR_RESERVE_USD + PROPOSAL_RESERVE_USD` and this constant sums only
   * the first term. The true field is $1.60 and 72 calls against $1.30 and 67.
   * Both figures clear the ceiling, so this test holds either way; it is
   * recorded here so a later correction of the constant is not read as a
   * regression.
   */
