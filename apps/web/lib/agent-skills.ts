...
export const dslSkill = [
  ...
  "part <name> [at <x>,<y>] [size <n> | fill] [turn cw|half|ccw] [flip]",
  ...
  "diamond <cx>,<cy> r<n>",
  "hole rect <x>,<y> <w>x<h> [r<n>]  |  hole circle <cx>,<cy> r<n>",
  "line <x>,<y> <x>,<y> ...     [r<n>] [off-axis] [solid] [detail]",
  "dot <x>,<y> <terminal|more|floating|node>",
  "subtract                     first of the last two filled groups minus second",
  "union                        merge the last two filled groups",
  ...
  "them exactly.",
  "",
  "Curves come from `circle`, `arc` and from parts, which carry the real curve",
  "data of the set they were extracted from. You name a centre, a radius, how",
  "far the arc travels and which side it starts from; the library computes the",
  "control points. Two half arcs facing opposite ways make an S.",
  "",
  "The DSL has exactly one escape, and it is asked for by name: `off-axis`",
  "allows one segment more than 6 degrees from an axis. It lands in the",
  "document and is visible in review.",
  "",
  "There is no way to write path data. The library has a `raw` entry point so",
  "that any existing icon can be imported into a document, but it is not an op",
  "and the parser refuses it. The list above is the whole language: an op that",
  "is not in it is an error, not a no-op.",
  "",
  "## Example",
  "",
  "```",
...
export const drawSkill = [
  ...
  "wants it, or move an endpoint onto the grid. Those are the only two moves —",
  "there is no escape that takes the coordinates verbatim, so a refused segment",
  "is a question about the shape rather than an obstacle to route around.",
  "",
