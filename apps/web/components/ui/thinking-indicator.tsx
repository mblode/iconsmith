...
const ShimmerText = ({
  ...
  // The sweep is the whole effect, and it is decoration. Without it the text
  // must still be text: a clipped transparent fill with no moving gradient
  // behind it renders nothing at all.
  if (reduce) {
    return <span className={cn("py-1 leading-[1.22] text-foreground", className)}>{children}</span>;
  }
  return (
    <motion.span
...
const ThinkingIndicator = ({
  ...
  // `motion` animates through inline styles and the Web Animations API, which
  // the global `prefers-reduced-motion` block in `globals.css` cannot reach —
  // that rule only zeroes CSS `animation-duration` and `transition-duration`.
  // So the guard has to live here, in JS, or this row animates regardless.
  const reduce = useReducedMotion();
