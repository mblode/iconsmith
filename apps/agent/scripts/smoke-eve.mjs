...
/**
 * A green smoke run must mean the field ran, not that something in it did.
 *
 * The assertions above are satisfied by `kind === "drawn"` on a library
 * recompile, and `home` has a house twin — so a run that refused BOTH drawing
 * arms for want of a reserve passed green while printing `strategy` for nobody
 * to read. That is exactly the failure the reservations caused in the campaign,
 * and the smoke test is the one place it should have been impossible to miss.
 */
const refused = output.tournament.strategy?.unaffordable ?? [];
if (refused.length > 0) {
  throw new Error(
    `The per-icon budget refused ${refused.length} arm(s) before they drew anything: ${refused.join(", ")}. ` +
      `Reserved $${output.tournament.strategy.budget?.reservedUsd ?? "?"} of $${output.tournament.strategy.budget?.maxUsd ?? "?"}.`,
  );
}
if (output.tournament.strategy?.haltedByFailure) {
  throw new Error(
    `An arm failed and stopped the tournament: ${output.tournament.candidates
      .filter((candidate) => candidate.failure)
      .map((candidate) => `${candidate.id}: ${candidate.failure}`)
      .join(" | ")}`,
  );
}
