...
/**
 * What the campaign is known to have spent, and what it must be BILLED for.
 *
 * These are not the same number, and every gate has to use the second one.
 * Studio's `measuredCost.totalUsd` is null whenever ANY call in the icon is
 * unpriced, so summing `cost.totalUsd ?? 0` booked the WHOLE icon at $0.00 for
 * one unpriced call. With the batch spend pinned at zero, `remainingSpend`
 * stayed at the full cap on every iteration and the per-icon clamp degenerated
 * to `maxIconSpend`: `--limit 200 --max-spend 5` would bill up to 200 x $2 =
 * $400 against a $5 cap — 80x — while the ledger printed $0.00. The signal was
 * already computed and discarded; `tournamentHasUnpricedCalls` is returned by
 * `statusOf` and was read nowhere.
 *
 * So an icon whose ledger is unknown is billed at the per-icon ceiling it ran
 * under, the only sound upper bound available: `clampIconBudget` in
 * `lib/studio/generate.ts` refuses to raise that ceiling and `exceedsCostBudget`
 * fails a run that crosses it. `knownTournamentUsd` still reports the measured
 * floor beside it, so the two numbers together say how much of the total is
 * evidence and how much is a bound.
 */
const costSummary = (campaign, unpricedIconUsd = DEFAULT_ICON_SPEND_USD) => {
  const costs = [...campaign.items, ...(campaign.retired ?? [])].flatMap(
    (item) => item.costs ?? [],
  );
  const knownTournamentUsd = costs.reduce((sum, cost) => sum + (cost.totalUsd ?? 0), 0);
  const unpricedIcons = costs.filter((cost) => cost.totalUsd === null).length;
  return {
    billedTournamentUsd: knownTournamentUsd + unpricedIcons * unpricedIconUsd,
    knownTournamentUsd,
    note: "Eve orchestration turns are not priced in the Studio tool result, and an icon whose tournament ledger is unknown is billed at the per-icon ceiling rather than at zero. The complete campaign total is bounded, not known.",
    totalUsd: null,
    tournamentCalls: costs.reduce((sum, cost) => sum + cost.calls, 0),
    tournamentHasUnpricedCalls: unpricedIcons > 0,
    /** Attempts with no usable ledger, each billed at `unpricedIconUsd`. A
     *  crashed turn is one of these: it never reaches `persistAttempt`, so it
     *  is booked unpriced rather than free. */
    unpricedIcons,
  };
};
...
const generate = async (root, campaign, options) => {
  ...
  // Billed, not known: an icon whose ledger came back unpriced is charged the
  // per-icon ceiling it ran under. Summing the known total instead is what let
  // one unpriced call book a whole $2 icon at $0.00 and hold the gate open.
  const spentSoFar = () => costSummary(campaign, maxIconSpend).billedTournamentUsd;
  const startingSpend = spentSoFar();
  const outcomes = [];

  for (const item of selected) {
    const batchSpend = spentSoFar() - startingSpend;
    ...
    try {
      ...
      item.status = "blocked";
      /**
       * A crashed turn books what it may have spent, not nothing.
       *
       * `persistAttempt` is the only place `item.costs` is appended and this
       * branch never reaches it, so a turn that died after the tournament had
       * already billed real calls left the batch gate believing the icon was
       * free — and a run that crashes repeatedly is exactly the run that
       * shouldn't be allowed to keep going. There is no ledger to read here,
       * the tool result never arrived, so it is recorded unpriced and billed at
       * the per-icon ceiling by the same rule as an unpriced tournament. The
       * call count stays 0 because that is also unknown, and inventing one
       * would corrupt `tournamentCalls`.
       */
      item.costs = [
        ...(item.costs ?? []),
        { calls: 0, records: [], totalUsd: null, unpricedCalls: 0 },
      ];
      await writeJson(path.join(checkpointRoot, `${safeName(item.slug)}.json`), {
        campaign: campaign.id,
    ...
    /**
     * The cap again, against what this attempt actually cost.
     *
     * Checking only at the top of the loop meant the attempt that crossed the
     * cap was never named — the next iteration broke silently, and the last
     * item in the batch was never checked at all. Reading it here also covers
     * the crash path above, which books its ceiling in the catch branch and
     * previously left the gate looking at a total that had not moved.
     */
    const billed = spentSoFar() - startingSpend;
    if (maxSpend !== null && billed >= maxSpend) {
      outcomes.push({
        billedUsd: Number(billed.toFixed(4)),
        reason: `Stopped after ${item.slug}: the batch has been billed $${billed.toFixed(2)} against the $${maxSpend.toFixed(2)} --max-spend cap.`,
        status: "stopped",
      });
      break;
    }
